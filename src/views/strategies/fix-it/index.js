...
function FixItStrategies() {
  ...
  const events =
    eventStore.getPositiveEventsForWeek( uiStore.currentWeek ) || [];
  const positiveFilter = events.filter( event => event.total > 0 );
  const initialBalance =
    positiveFilter.find( event => event.category === 'income.startingBalance' );
  const positiveEvents =
    positiveFilter.filter( event => event.category !== 'income.startingBalance' && event.category !== 'income.benefits.snap' );
  const weekIncome =
    positiveEvents.reduce( ( acc, event ) => acc + event.total, 0 );
  const negativeFilter =
    events.filter( event => event.total < 0 );
  const weekExpenses =
    negativeFilter.reduce( ( acc, event ) => acc + event.total, 0 );
  return (
    <section className='strategies'>
      ...
      <header className='strategies-header'>
        <div className='strategy-cards'>
          <h2 className='strategies-header__week-range'>
            Week of {uiStore.weekRangeText}
          </h2>
          <CardGroup columns={2}>
            <div className='fixit-header'>
              <div className='fixit-header__line-first'>
                <div>Amount that you went over: </div>
                <div className='fixit-header__amount'>
            ...
            <div className='fixit-header'>
              <div className='fixit-header__comment'>
                ...
                <div className='fixit-header__comment-value'>
                  {initialBalance ?
                    formatCurrency( initialBalance.total ) :
            ...
            <div className='fixit-header'>
              <div className='fixit-header__comment'>
                <div>You currently have a SNAP balance of {uiStore.weekEndingSnapBalanceText}.
                </div>
              </div>
            </div>
          </CardGroup>
      ...
      <div>{strategies.fixItResults.length > 0 &&
        <StrategyCards results={strategies.fixItResults} />}
      </div>
      <div>
        <Strategies />
      </div>
